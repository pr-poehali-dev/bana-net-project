                    ...
                    <CardContent className="flex items-center justify-between py-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {formatDate(t.updated_at)} · {t.message_count} сообщ.
                        </p>
                      </div>
              ...
              <Card className="mt-6">
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm">
                      Перейдите в раздел «Добавить отзыв», заполните все
                      обязательные поля и прикрепите минимум 2 изображения.
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-muted-foreground text-sm">
                      Обычно 24-48 часов. В редких случаях может занять до 3-х
                      рабочих дней.
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold mb-1">
                      Можно ли удалить свой отзыв?
                    </h4>
                    <p className="text-muted-foreground text-sm">
                      Нет, удаление отзывов недоступно. Вы можете
                      отредактировать отзыв в личном кабинете.
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Обращение #{selected.id} · {formatDate(selected.created_at)}
                </span>
              </div>
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {(selected.messages || []).map((m) => (
                    ...
                    >
                      <div
                        className={`max-w-[85%] rounded-xl px-4 py-3 text-sm ${m.is_admin ? "bg-muted" : "bg-primary text-primary-foreground"}`}
                      >
                        <p
                          className={`text-xs font-medium mb-1 ${m.is_admin ? "text-muted-foreground" : "text-primary-foreground/70"}`}
                        >
                          {m.is_admin ? "Поддержка" : m.author_name}
                        </p>
                        <p className="whitespace-pre-wrap">{m.body}</p>
                        <p
                          className={`text-xs mt-1.5 ${m.is_admin ? "text-muted-foreground" : "text-primary-foreground/60"}`}
                        >
